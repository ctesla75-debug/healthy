...
async function updateRangeStats(){
  const all = await getAllLogs();
  if(!all.length){
    document.querySelector("#recordRange").textContent="No data yet";
    return;
  }
  ...
  if(days>0) text += (text?" ":"")+days+" day"+(days>1?"s":"");
  if(!text) text="1 day";

  document.querySelector("#recordRange").textContent = "Data span: "+text;
}
